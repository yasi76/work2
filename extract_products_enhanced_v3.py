#!/usr/bin/env python3
"""
Enhanced Digital Health Product Extractor V3
Addresses core issues: UI noise, type detection, GT matching, and scraping coverage
"""
...
PRODUCT_TYPE_PATTERNS = {
    ...
    'service': {
        ...
        'weight': 0.5,  # Lower weight to avoid over-classification
        'force_keywords': []
    },
    'ai_tool': {
...
class EnhancedProductExtractor:
    def __init__(self, timeout: int = 30, use_js: str = 'auto', debug: bool = False):
        ...
        self.product_paths = [
            ...
            '/modules', '/module', '/components', '/use-cases', '/anwendungen'
        ]
    ...
    def extract_all_products(self, url: str, company_name: str = None) -> Dict:
        ...
        for path in self.product_paths[:10]:  # Limit to avoid too many requests
        ...
        return {
            ...
            'pages_urls': pages_scraped[:5]  # First 5 for reference
        }
    ...
    def _parse_html_for_products(self, html_content: str, url: str) -> List[Dict]:
        ...
        # Method 1: Schema.org structured data
        ...
        # Method 3: Headings with context
        heading_products = self._extract_from_headings_with_context(soup)
        products.extend(heading_products)
        
        # Method 4: Lists in product sections
        list_products = self._extract_from_product_lists(soup)
        products.extend(list_products)
        
        # Method 5: Meta tags and Open Graph
            ...
            try:
                ...
                for item in schema_items:
                    if isinstance(item, dict) and item.get('@type') in ['Product', 'SoftwareApplication', 'MedicalDevice', 'MobileApplication']:
                        ...
                        if name and not UI_NOISE_BLACKLIST.match(name):
                            products.append({
                                'name': name,
    ...
    def _extract_from_cards_strict(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        card_selectors = [
            ...
            '.card.product', '.tile.product', '[class*="product-card"]',
            '.solution-card', '.service-card', '.app-card',
            'article.product', 'div[data-product]', '.portfolio-item'
        ]
        
        for selector in card_selectors:
            for card in soup.select(selector):
                # Look for product name in card
                name_elem = card.select_one('h2, h3, h4, .title, .name, .product-name')
                if name_elem:
                    name = name_elem.get_text(strip=True)
                    
                    # Strict filtering
                    if (name and 
                        not UI_NOISE_BLACKLIST.match(name) and
                        len(name) > 2 and 
                        len(name) < 50 and
                        not name.isdigit()):
                        
                        # Get context from card
                        context = card.get_text(strip=True)[:200]
    ...
    def _extract_from_headings_with_context(self, soup: BeautifulSoup) -> List[Dict]:
        """Extract from headings with surrounding context"""
        products = []
        
        # Skip headings in navigation, footer, header
        ...
        for tag in ['h2', 'h3', 'h4']:
            for heading in soup.find_all(tag):
                ...
                # Enhanced filtering
                if (text and 
                    not UI_NOISE_BLACKLIST.match(text) and
                    len(text) > 2 and 
                    len(text) < 50 and
                    ...
                    # Check if it's in a product-related section
                    section_text = context.lower()
                    if any(keyword in section_text for keyword in ['product', 'solution', 'feature', 'offering', 'portfolio']):
                        confidence = 0.7
                    else:
                        confidence = 0.5
                    
                    # Boost confidence if has product indicators
                    if any(indicator in text for indicator in ['App', 'Set', 'System', 'Platform', 'Assistant']):
    ...
    def _extract_from_product_lists(self, soup: BeautifulSoup) -> List[Dict]:
        """Extract from lists in product sections"""
        products = []
        
        # Find product-related sections
        product_sections = soup.find_all(['section', 'div'], 
            class_=re.compile(r'product|solution|offering|portfolio', re.I))
        ...
        for section in product_sections:
            ...
            for list_elem in section.find_all(['ul', 'ol']):
                for li in list_elem.find_all('li'):
                    ...
                    # Extract first meaningful part (often the product name)
                    parts = text.split('-')
                    if parts:
                        name = parts[0].strip()
                        ...
                        if (name and 
                            ...
                            products.append({
                                'name': name,
                                'type': self._classify_product_type_enhanced(name, text),
                                'confidence': 0.6,
                                'method': 'product_list'
                            })
    ...
    def _extract_from_meta_enhanced(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        meta_properties = [
            'og:title', 'og:site_name', 'twitter:title', 
    ...
    def _classify_product_type_enhanced(self, product_name: str, context: str) -> str:
        ...
        return 'software'  # Better default than 'service'
    ...
    def _fuzzy_match_product(self, name1: str, name2: str, threshold: float = 0.8) -> bool:
        ...
        # Token-based match (all tokens from shorter name in longer)
    ...
    def _deduplicate_and_filter(self, products: List[Dict]) -> List[Dict]:
        ...
        for product in products:
            ...
            if UI_NOISE_BLACKLIST.match(name):
                logger.debug(f"Filtered out UI noise: {name}")
                continue
        ...
        if self.debug:
            for product in final_products:
                logger.debug(f"Product: {product['name']}, Type: {product['type']}, "
                           f"Confidence: {product['confidence']:.2f}, Method: {product['method']}")
        
        return final_products


def process_startup(startup_data: Dict, extractor: EnhancedProductExtractor) -> Dict:
    ...
    try:
        ...
        # Format for output
        product_names = [p['name'] for p in products[:5]]  # Top 5
        ...
        output_data = {
            ...
            'top_products': products[:3],  # Top 3 with full details
...
def main():
    parser = argparse.ArgumentParser(description='Enhanced Product Extractor V3')
    parser.add_argument('input_file', help='JSON file with startup data')
    parser.add_argument('--output-prefix', default='products_v3', help='Output file prefix')
